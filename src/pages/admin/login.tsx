...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    // Simulate a brief check — the actual token validation happens on first API call
    setTimeout(() => {
      login(token.trim());
      toast.success("登录成功");
      navigate("/admin", { replace: true });
    ...
    <div className="min-h-screen flex items-center justify-center bg-muted/30 px-4">
      <Card className="w-full max-w-sm rounded-3xl shadow-sm border-border/50">
        <CardHeader className="text-center pb-2">
          ...
          <CardTitle className="text-xl tracking-tight">管理后台</CardTitle>
          <CardDescription>请输入您的访问令牌以继续</CardDescription>
        </CardHeader>
        <CardContent>
